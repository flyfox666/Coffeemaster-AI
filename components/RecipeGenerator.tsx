...
import { Sparkles, Coffee, ChefHat, Clock, Info, Loader2, Image as ImageIcon, Camera } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-coffee-200 overflow-hidden flex flex-col min-h-[600px]">
      ...
      <div className="p-6 flex-1 flex flex-col">
        {step === 'input' && (
          <div className="flex flex-col h-full justify-center items-center space-y-6 py-4">
            <div className="w-full max-w-md space-y-4">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl border border-coffee-200 focus:ring-2 focus:ring-coffee-500 focus:border-transparent h-32 resize-none bg-coffee-50 text-coffee-900 placeholder-coffee-300"
              />
              ...
              <button
                ...
                className="w-full bg-coffee-600 hover:bg-coffee-700 text-white font-medium py-3 px-6 rounded-xl flex items-center justify-center gap-2 transition-all transform hover:scale-[1.02] shadow-md mt-4"
              >
        ...
        {step === 'result' && recipe && (
          <div className="space-y-8 animate-fadeIn">
            {/* Image Section */}
            <div className="relative w-full h-64 rounded-xl overflow-hidden bg-coffee-100 shadow-inner group border border-coffee-100">
              {imageUrl ? (
                <img 
                  src={imageUrl} 
                  alt={recipe.title} 
               ...
               <h4 className="font-bold text-coffee-800 mb-4 flex items-center gap-2 text-lg">
                ...
                <div className="space-y-6">
                  {recipe.steps.map((step, i) => (
                    <div key={i} className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-xl border border-coffee-100 shadow-sm">
                       {/* Step Image Container */}
                       <div className="w-full md:w-32 h-32 rounded-lg bg-coffee-50 shrink-0 overflow-hidden border border-coffee-100 relative">
                          {stepImages[i] ? (
                             <img src={stepImages[i]} alt={`Step ${i+1}`} className="w-full h-full object-cover" />
                          ) : (
                          ...
                          <div className="absolute top-0 left-0 bg-coffee-600 text-white text-xs font-bold px-2 py-1 rounded-br-lg">
                             {i + 1}
                          </div>
                       </div>
